...
while va < vmax:
  ...
  [odata,it,res] = DDGgummelmap (n_max,xin,idata,odata,toll,maxit,ptoll,pmaxit,verbose)
  ...
  Emax[istep] = max(abs((V_[istep, 1:n_max]-V_[istep,0:n_max-1])/(xin[1:n_max]-xin[0:n_max-1])))
  #
  Bp = Ubernoulli( (V_[istep, 1:n_max]-V_[istep, 0:n_max-1]),1)
  Bm = Ubernoulli((V_[istep, 1:n_max]-V_[istep, 0:n_max-1]),0)
  Jn[istep,:] = -odata.mun * (n_[istep, 1:n_max]*Bp-n_[istep, 0:n_max-1]*Bm)/ (xin[1:n_max]-xin[0:n_max-1])
  Jp[istep,:] =  odata.mup * (p_[istep, 1:n_max]*Bm-p_[istep, 0:n_max-1]*Bp)/ (xin[1:n_max]-xin[0:n_max-1])
...
pl.suptitle('1D Drift Diffusion Model for pn Diodes Results - at Applied Bias (%.2f)V'%vvect[istep-1], fontsize=12)
pl.subplots_adjust(hspace=0.4,wspace=0.4)
pl.subplot(2,2,1)
pl.plot(vvect[0:istep-1], abs(Jp[0:istep-1,2]+Jn[0:istep-1,2])*us * q * ns,'b')
pl.xlabel('v [V]')
pl.ylabel('J [A]')
