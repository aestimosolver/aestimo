...
def run_example(file_path):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=30, # Short timeout to fail fast
            env=env
        )
...
def main():
    ...
    # Skip the first 2 examples as they are known to pass/be slow
    files_to_test = python_files[2:]
    
    total = len(files_to_test)
    passed = 0
    failed = []
    
    print(f"Starting tests for {total} examples (skipped first 2)...\n", flush=True)
