...
def Poisson_equi_non_2(vindex,fitotc,fitot,Nc,Nv,fi_e,fi_h,n,p,dx,Ldi,dop,Ppz_Psp,pol_surf_char,ni,n_max,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,surface,fi_stat):
    ...
    if config.predic_correc:#(check high concentrations)
        ...
        n,p,fi_non =equi_np_fi22(vindex,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_out,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni,n,p)
...
def fd3(x):
    ...
    """
    return 1/(exp(-x)+3/4*sqrt(pi)*(x**4+ 50 + 33.6*x*(1-0.68*exp(-0.17*(x+1)**2)))**(-3/8))
def fd4(x):
    """
    Approximation used for the Fermi integral by:
...
def fd6(x):
    ...
    """    
    return log(x)/(x**2-1)+(3*sqrt(pi)*x/4)**(2/3)+(2*3*sqrt(pi)*x/4)/(3+3*sqrt(pi)*x/4)**2
def fd7(x):
    """
    Approximation used for the Fermi integral by:
    D. Bednarczyk and J. Bednarczyk, The approximation of the Fermi-Dirac
    integral f1/2(η), Phys. Lett., vol. 64A, pp. 409–410, 1978.
    """
    return (exp(-x)+3/4*sqrt(pi)*(x**4+ 50 + 33.6*x*(1-0.68*exp(-0.17*(x+1)**2)))**(-3/8))
...
def equi_np_fi2(fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni):#use    
    n=np.zeros(n_max)
    n_cl=np.zeros(n_max)
    n_qw=np.zeros(n_max)
    p=np.zeros(n_max)
...
def equi_np_fi4(fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni):#use    
    n=np.zeros(n_max)
    n_cl=np.zeros(n_max)
    n_qw=np.zeros(n_max)
    p=np.zeros(n_max)
...
def Poisson_equi1(dx,dop,fi,n_max):
    ...
    for i in  range(0,n_max):
        a[i]=1./(dx2)
        c[i]=1./(dx2)
        b[i]=-(2./(dx2)+exp(fi_out[i])+exp(-fi_out[i]))
        f[i]=exp(fi_out[i])-exp(-fi_out[i])-dop_out[i]-fi_out[i]*(exp(fi_out[i])+exp(-fi_out[i]))
    # (B) Define the elements of the coefficient matrix and initialize the forcing
    # function at the ohmic contacts
    a[0]=0.
    c[0]=0.
...
def Current1(vindex,n,p,mun,mup,fi,Vt,n_max,Total_Steps,q,dx,ni,Ldi,Jnip1by2,Jnim1by2,Jelec,Jpip1by2,Jpim1by2,Jhole):
    ##########################################################################
    ##                        CALCULATE CURRENT                             ##
    ##########################################################################             
    for i in range(1,n_max-1):
...
def Write_results_equi2(ns,fitotc,fitot,Vt,q,ni,n,p,dop,dx,Ldi,fi,n_max):
    ...
    fi_result=fi[0:n_max]
    return Ec_result,Ev_result,ro_result,el_field1_result,el_field2_result,nf_result,pf_result,fi_result

def Write_results_non_equi1(dEc,Vt,q,ni,n,p,dop,dx,Ldi,fi,n_max
    ...
    nf_result=n[0:n_max]*ni
    pf_result=p[0:n_max]*ni
    return fi_result,Efn_result,Efp_result,ro_result,el_field1_result,el_field2_result,nf_result,pf_result,Ec_result,Ev_result,Ei_result,axis,av_curr
...
def Continuity2(n,p,mun,mup,fi,Vt,Ldi,n_max,dx,TAUN0,TAUP0):               
    ...
    bp[0] = 1              #Co-ef for hole     at Anode
    cp[0] = 0              #Co-ef for hole     at Anode
    fnp[0] = (dx2/Vt)*(p[0]*n[0]-1)/(TAUP0[0]*(n[0]+1)+TAUN0[0]*(p[0]+1))
    fn[0] = n[0]
    fp[0] = p[0]                
    an[n_max-1] = 0          #Co-ef for electron at Cathode
    bn[n_max-1] = 1          #Co-ef for electron at Cathode
    ...
    bp[n_max-1] = 1          #Co-ef for hole     at Cathode
    cp[n_max-1] = 0          #Co-ef for hole     at Cathode
    fnp[n_max-1] = (dx2/Vt)*(p[n_max-1]*n[n_max-1]-1)/(TAUP0[n_max-1]*(n[n_max-1]+1)+TAUN0[n_max-1]*(p[n_max-1]+1))
    fn[n_max-1] = n[n_max-1]
    fp[n_max-1] = p[n_max-1]
    #(B) Define the elements of the coefficient matrix for the internal nodes and
    #    initialize the forcing function
    for i in range(1,n_max-1): 
        ...
        bn[i]=-(munim1by2*Ber((fi[i]-fi[i-1]))+munip1by2*Ber(fi[i]-fi[i+1]))
        ## Forcing Function for ELECTRON and HOLE Continuity eqns
        fn[i] = (dx2/Vt)*(p[i]*n[i]-1)/(TAUP0[i]*(n[i]+1)+TAUN0[i]*(p[i]+1))
        fp[i] = (dx2/Vt)*(p[i]*n[i]-1)/(TAUP0[i]*(n[i]+1)+TAUN0[i]*(p[i]+1))   
    #(C)  Start the iterative procedure for the solution of the linearized Continuity
    #     equation for "ELECTRONS" using LU decomposition method:
    dn[0] = bn[0]
    for i in range (1,n_max):
...
def Mobility3(mun0,mup0,fi,fi_n,fi_p,Vt,Ldi,VSATN,VSATP,BETAN,BETAP,n_max,dx):
    #######################################################################
    #% 3.1 . Calculate Field Dependant Mobility for each value of 'fi'   ## 
    #%       at each node point of the PN diode.                         ##
    #######################################################################
...
def Continuity3(n,p,mun,mup,fi,fi_n,fi_p,Vt,Ldi,n_max,dx,TAUN0,TAUP0):               
    ...
    for i in range(1,n_max-1): 
        ...
        bp[i]=-(mupim1by2*Ber((fi[i-1]-fi[i])+(fi_p[i-1]-fi_p[i]))+mupip1by2*Ber((fi[i+1]-fi[i])+(fi_p[i+1]-fi_p[i])))
        ## Co-efficients for ELECTRON Continuity eqn
        an[i]=munim1by2*Ber((fi[i-1]-fi[i])+(fi_n[i-1]-fi_n[i]))
        cn[i]=munip1by2*Ber((fi[i+1]-fi[i])+(fi_n[i+1]-fi_n[i]))
        bn[i]=-(munim1by2*Ber((fi[i]-fi[i-1])+(fi_n[i]-fi_n[i-1]))+munip1by2*Ber((fi[i]-fi[i+1])+(fi_n[i]-fi_n[i+1])))
        ## Forcing Function for ELECTRON and HOLE Continuity eqns
        fn[i] = (dx2/Vt)*(p[i]*n[i]-1)/(TAUP0[i]*(n[i]+1)+TAUN0[i]*(p[i]+1))
        fp[i] = (dx2/Vt)*(p[i]*n[i]-1)/(TAUP0[i]*(n[i]+1)+TAUN0[i]*(p[i]+1))
    #(C)  Start the iterative procedure for the solution of the linearized Continuity
    ...
    ####################### END of HOLE Continuty Solver ###########
    return n,p
def Poisson_non_equi2(fi_stat,n,p,dop,Ppz_Psp,pol_surf_char,n_max,dx,fi,flag_conv_2,Ldi,ni,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general):
    #################################################################### 
    ## 3.3 Calculate potential fi again with new values of "n" and "p"##
    ##     and check for convergence                                  ##     
    ####################################################################                
...
def equi_np_fi31(g,fi_stat31,n,p,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni):#use
    EF=0.0
    nf=n*ni
    pf=p*ni
    ...
    for k in range(1,model.N_wells_virtual-1):
        ...
        for jj in range(0,model.subnumber_h,1):            
            for ii in range(I1,I2):
                ...
                Delta_fi[ii]=-Vt*q*fi_stat31[ii]-(-Vt*q*fi_old[ii])#predictor-corrector-type approach.
    ...
    for k in range(1,model.N_wells_virtual-1):
        I1,I2,I11,I22 =amort_wave(k,model.Well_boundary,n_max)
        for j in range(0,model.subnumber_e,1):  
            E_statec_general_pc[k,j]=E_statec_general[k,j]-Delta_fi[I1:I2].mean()*J2meV
        for jj in range(0,model.subnumber_h,1):            
            E_state_general_pc[k,jj]=E_state_general[k,jj]-Delta_fi[I1:I2].mean()*J2meV
        #print(Delta_fi[I1:I2])
        #print(fi_old[I1:I2])        
     
    xaxis = np.arange(0,n_max)*model.dx 
    span=np.ones(100000000)        
    pl.plot(xaxis,Ev*1e3,'k',xaxis,Ec*1e3,'k')#,xaxis,Efn*1e3,'r',xaxis,Efp*1e3,'b')
    ...
    """"""
    return n,p,fi_old,EF  # density of carriers
def equi_np_fi22(vindex,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni,n,p):#use    
    n_q=np.zeros(n_max)
    p_q=np.zeros(n_max)
    Ec=np.zeros(n_max)
    Ev=np.zeros(n_max)
...
def Poisson_non_equi3(vindex,fi_stat3,n,p,dop,Ppz_Psp,pol_surf_char,n_max,dx,fi,flag_conv_2,Ldi,ni,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general):
    ...
    if model.N_wells_virtual-2!=0 and config.quantum_effect:                    
         n_q,p_q,fi_n,fi_p =equi_np_fi22(vindex,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_out,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,Ppz_Psp,n_max,ni,n,p)
...
def Current2(vindex,n,p,mun,mup,fi,Vt,n_max,Total_Steps,q,dx,ni,Ldi,Jnip1by2,Jnim1by2,Jelec,Jpip1by2,Jpim1by2,Jhole):
    ##########################################################################
    ##                        CALCULATE CURRENT                             ##
    ##########################################################################             
    for i in range(1,n_max-1):
        ...
        # Electron Current
        Jnip1by2[vindex,i] = (q*mun[i]*Vt/(dx)) * ni[i]*( n[i+1]*Ber((fi[i+1]-fi[i]))- n[i]*Ber((fi[i]-fi[i+1])) )
        Jnim1by2[vindex,i] = (q*mun[i]*Vt/(dx)) * ni[i]*( n[i]*Ber((fi[i]-fi[i-1]))- n[i-1]*Ber((fi[i-1]-fi[i])) )
        Jelec[vindex,i] = (Jnip1by2[vindex,i] + Jnim1by2[vindex,i])/2
        #Hole Current
        Jpip1by2[vindex,i] = (q*mup[i]*Vt/(dx)) * ni[i]*( p[i+1]*Ber((fi[i]-fi[i+1]))- p[i]*Ber((fi[i+1]-fi[i])) )
        Jpim1by2[vindex,i] = (q*mup[i]*Vt/(dx)) * ni[i]*( p[i]*Ber((fi[i-1]-fi[i])) - p[i-1]*Ber((fi[i]-fi[i-1])) )
        Jhole[vindex,i] = (Jpip1by2[vindex,i] + Jpim1by2[vindex,i])/2
    ##         Jtotal(vindex) = Jelec
    return Jnip1by2,Jnim1by2,Jelec,Jpip1by2,Jpim1by2,Jhole
...
def Write_results_non_equi2(Nc,Nv,fi_e,fi_h,Vt,q,ni,n,p,dop,dx,Ldi,fi,n_max
    ...
    nf_result=n[0:n_max]*ni[0:n_max]
    pf_result=p[0:n_max]*ni[0:n_max]
    return fi_result,Efn_result,Efp_result,ro_result,el_field1_result,el_field2_result,nf_result,pf_result,Ec_result,Ev_result,Ei_result,av_curr
