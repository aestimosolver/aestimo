...
def Poisson_equi2(fitotc,fitot,Nc,Nv,fi_e,fi_h,n,p,dx,Ldi,dop,ni,n_max,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,surface,fi_old0):
    ...
    if iteration==1:
        ...
        p=np.zeros(n_max)
        fi_out=np.zeros(n_max)
        n,p,fi_out=equi_np_fi(iteration,dop,n_max,ni,model,Vt,surface)
        fi_old0=fi_out
    else:
        n=np.zeros(n_max)
        p=np.zeros(n_max)
...
def equi_np_fi2(fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,n_max,ni):#use    
    n=np.zeros(n_max)
    n_cl=np.zeros(n_max)
    n_qw=np.zeros(n_max)
    p=np.zeros(n_max)
...
def equi_np_fi3(fi_old0,n,p,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,fi_old,Vt,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general,dop,n_max,ni):#use
    ...
    for k in range(1,model.N_wells_virtual-1):
        ...
        for j in range(0,model.subnumber_e,1):
            for i in range(I1,I2):
                Delta_fi[i]=-Vt*q*fi_old0[i]-(-Vt*q*fi_old[i])
        ...
        for jj in range(0,model.subnumber_h,1):          
            for ii in range(I1,I2):
                Delta_fi[ii]=-Vt*q*fi_old0[ii]-(-Vt*q*fi_old[ii])#predictor-corrector-type approach.
...
def equi_np_fi(iteration,dop,n_max,ni,model,Vt,surface):                
    n=np.zeros(n_max)
    p=np.zeros(n_max)
...
def Poisson_non_equi2(fi_old0,n,p,dop,n_max,dx,fi,flag_conv_2,Ldi,ni,fitotc,fitot,Nc,Nv,fi_e,fi_h,iteration,wfh_general,wfe_general,model,E_state_general,E_statec_general,meff_state_general,meff_statec_general):
    #################################################################### 
    ## 3.3 Calculate potential fi again with new values of "n" and "p"##
    ##     and check for convergence                                  ##     
    ####################################################################                
...
def Write_results_non_equi2(fi_e,fi_h,Vt,q,ni,n,p,dop,dx,Ldi,fi,n_max
    ...
    Ev[0] = Ev[1]
    Ev[n_max-1] = Ev[n_max-2]
    axis[n_max-1] = axis[n_max-2] + dx*1e4
    el_field1[0] = el_field1[1]
    el_field2[0] = el_field2[1]
    el_field1[n_max-1] = el_field1[n_max-2]
    el_field2[n_max-1] = el_field2[n_max-2]
    ...
    for i in range(0,n_max):
        Ei[i]   = Ec[i] -((fi_e[i]-fi_h[i])/(2*q))
        Efn[i]  = Ei[i] + Vt*log(nf[i]/ni[i])
        Efp[i]  = Ei[i] - Vt*log(pf[i]/ni[i])           
